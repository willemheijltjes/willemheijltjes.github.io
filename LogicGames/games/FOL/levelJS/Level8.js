...
let context = [];
let sentence1 = {type:"predicate", formula:"Logician", args:["Frege"]};
let sentence2 = {type:"predicate", formula:"Likes", args:["Frege", "Aristotle"]};
...
function makeContext(){
    ...
    if (russellEmotion){
        context.push(russellEmotion+"(Frege )");
    }
    if(fregeEmotion){
        context.push(fregeEmotion+"(Frege )");
    }
    if(russellFrege){
        context.push(russellFrege+"(Frege Aristotle )");
    }
...
function drop(ev) {
    ...
    if (String(data)=="Happy" || String(data)=="HasTheory" || String(data)=="Logician" || String(data)=="Sad" || String(data)=="HasIdea"){
